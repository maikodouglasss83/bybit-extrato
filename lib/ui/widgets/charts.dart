...
/// Uma barra do gráfico horizontal do planejamento.
class HorizontalBarData {
  ...
  /// Meta da categoria. Zero quando não há meta definida.
  final double target;

  final Color color;
}

/// Gráfico de barras horizontais: cada categoria vira uma barra, com a meta
/// desenhada como trilho ao fundo e um traço marcando onde ela cai.
class HorizontalBarChart extends StatelessWidget {
  ...
  /// Texto à direita de cada barra, já formatado na moeda escolhida.
  final String Function(HorizontalBarData) labelBuilder;
  ...
  Widget build(BuildContext context) {
    if (bars.isEmpty) {
      return Padding(
        ...
        child: Center(
          child: Text('Nada gasto neste mês', style: context.texts.bodySmall),
        ),
      );
    }

    // A escala é comum a todas as barras: comparar categorias só faz sentido
    // se a mesma largura valer o mesmo dinheiro em todas.
    final maior = bars.fold<double>(
      0,
      (m, b) => math.max(m, math.max(b.value, b.target)),
    );
...
class _Bar extends StatelessWidget {
  const _Bar({required this.bar, required this.scale, required this.trailing});

  final HorizontalBarData bar;
  final double scale;
  ...
  Widget build(BuildContext context) {
    final tones = context.tones;
    final estourou = bar.target > 0 && bar.value > bar.target;
    final cor = estourou ? tones.negative : bar.color;
      ...
      children: [
          ...
          children: [
            ...
            Text(
              ...
              style: context.texts.titleSmall?.copyWith(
                color: estourou ? tones.negative : null,
              ),
            ),
          ],
        ),
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            final fracaoGasto = scale <= 0 ? 0.0 : (bar.value / scale).clamp(0.0, 1.0);
            final fracaoMeta = scale <= 0 ? 0.0 : (bar.target / scale).clamp(0.0, 1.0);
            ...
            return SizedBox(
              ...
              child: Stack(
                children: [
                  ...
                  if (bar.target > 0)
                    Positioned(
                      left: (largura * fracaoMeta - 1).clamp(0.0, largura - 2),
                      top: -2,
                      bottom: -2,
                      child: Container(
                        width: 2,
