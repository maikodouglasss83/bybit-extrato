...
void main() {
  ...
  test('a lista vai do dia 1 ao 31, com as canceladas no dia delas', () async {
    final state = AppState();
    // Cadastradas fora de ordem de propósito. O dia 28 pode vencer antes do
    // dia 1 no calendário, mas a lista segue o dia do mês.
    await state.addManualSubscription(
        name: 'Dia 28', monthlyBrl: 500, dueDay: 28);
    await state.addManualSubscription(
        name: 'Dia 1', monthlyBrl: 10, dueDay: 1);
    await state.addManualSubscription(
        name: 'Dia 15 cancelada', monthlyBrl: 50, dueDay: 15);
    await state.addManualSubscription(
      name: 'Dia 10 pela cobrança',
      monthlyBrl: 20,
      lastCharge: DateTime(2026, 8, 10),
    );
    await state.addManualSubscription(
        name: 'Dia 1 mais cara', monthlyBrl: 99, dueDay: 1);
    ...
    final cancelada =
        state.subscriptions().firstWhere((s) => s.name == 'Dia 15 cancelada');
    ...
    expect(state.subscriptionsMonthlyBrl, closeTo(934, 0.001));
  });
